  ...
  if (matched.tiles.length > 0) {
    ...
    matched.tiles.forEach(tile => {
      if (data[tile[0]][tile[1]] !== 0) {
        ...
        score += !isNaN(parseInt(matchedTile.textContent)) ? parseInt(matchedTile.textContent) : 0;
        scoreTxtObj.textContent = score;
        ...
        if(score > parseInt(highscoreTxtObj.textContent))
        {
          ...
          highscoreTxtObj.style.animation = "addScore 0.5s ease-in-out 0s 1 forwards";
        }
...
const saveScore = () => {
  document.cookie = "highscore=" + highscoreTxtObj.textContent + "; max-age=3600; path=/";
}
